function graphTrace(options) {
	...
	// 获取图片资源
	var imageUrl = opts.ctx + "/process/process-diagram.action?processInstanceId=" + opts.pid;
	$.getJSON(opts.ctx + "/process/process-diagram-detail.action?processInstanceId=" + opts.pid, function(infos) {
		...
		if ($('#workflowTraceDialog').length == 0) {
			$(
					'<div/>',
					{
						...
						html : "<div><img src='" + imageUrl + "' style='position:absolute; left:0px; top:0px;' />" + "<div id='processImageBorder'>"
		...
		// 设置每个节点的data
		$('#workflowTraceDialog .activityAttr').each(function(i, v) {
