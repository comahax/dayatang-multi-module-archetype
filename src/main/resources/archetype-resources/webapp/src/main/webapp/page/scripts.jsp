...
proj_app = {
    ...
    get_currentuser_roles: function (options) {
        var settings = ${symbol_dollar}.extend({
    ...
    assigned_role: function (inputId, options) {
        ...
        function data4TreeData(orgs) {
            ...
            var results = [];
            ${symbol_dollar}.each(orgs, function (i, v) {
                var item = {id: '', text: ''};
                item.id = v.id;
    ...
    constructings_project_selector: function (inputId, init_id,options) {
        ...
        var settings = ${symbol_dollar}.extend({
            url: "${symbol_dollar}{basePath}/project/constructings-json.action"
        }, options || {});
            ...
            toolbar: [
                {
                    ...
                    iconCls: 'icon-search',
                    handler: function () {
                        ${symbol_dollar}.messager.prompt('搜索', '请输入项目的关键字:', function (r) {
    ...
    init_permissions_tree: function (ulElement, options, role_id) {
        ...
        ${symbol_dollar}.fn.zTree.init(${symbol_dollar}(ulElement), settings, data);
    ...
    convert_permission_serverdata_to_clientdata: function (data) {
        ...
        ${symbol_dollar}.each(data, function (i, v) {
            var node = v;
            node.isParent = true;
            result.push(node);
        });
    ...
    organizational_scope: function (inputId, options) {
        ...
        function treeExpendHandler(node) {
            ${symbol_dollar}.getJSON(settings.url + node.id, function (childData) {
    ...
    organization_info_selector: function (inputId, options, init_info) {
        ...
        if (!org.attr('contact_input_id')) {
            ...
            var contacter = ${symbol_dollar}('<input/>').attr('name', settings.contractInputName).attr('id', contact_input_id);
            org.after(contacter);
        } else {
            contact_input_id = org.attr('contact_input_id');
        }

        var contacter = ${symbol_dollar}('${symbol_pound}' + contact_input_id);
        ...
        function orgChange(newValue, oldValue) {
            ...
            if (init_info && init_info.person) {
                ${symbol_dollar}.each(data.results, function (i, v) {
    ...
    area_selector: function (inputId, options) {
        ...
        //单击树，展开树的事件处理
        function treeExpendHandler(node) {
            ${symbol_dollar}.getJSON(areaJsonUrl + node.id, function (childData) {
                if (!childData || !childData.results) {
                    return;
                }
        ...
        function areaData4TreeData(areas) {
            ...
            var results = [];
            ${symbol_dollar}.each(areas, function (i, v) {
                var item = {id: '', text: '', state: 'closed', children: []};
                item.id = v.id;
                item.text = v.name;
                item.attributes = v;
    ...
    aptype_selector: function (inputId) {
        ${symbol_dollar}(inputId).combobox({
            valueField: 'serialNumber',
            textField: 'text'
        });
        ...
        var data = getJSON(url);
        ${symbol_dollar}(inputId).combobox({data: data.results});
    ...
    start_finished_date_box: function (options) {
        ...
        if (startbox) {
            ...
            finishedbox.datebox({
                ...
                validType: "dateLtOther['${symbol_pound}" + startbox_id + "']",
    ...
    designorg_selector: function (inputId, options) {
        ...
        var contacter = ${symbol_dollar}('<input/>').attr('name', settings.contractInputName);
        org.after(contacter);
...
var imessager = {
    alert: function (options) {
        if ('string' == typeof options) {
            ${symbol_dollar}.messager.alert("提示", options);
            return;
        }
        ...
        ${symbol_dollar}.messager.alert(settings.title, settings.content);
    },
    success_tip: function () {
        this.tip('操作成功');
    },
    tip: function (content) {
        ${symbol_dollar}.messager.show({
            title: '提示',
            msg: content,
            timeout: 1500,
            showType: 'slide'
    ...
    confirm: function (question, callback, title) {
        ${symbol_dollar}.messager.confirm(title || "请确认", question, callback);
    },
    fail_tip: function () {
        this.alert('操作失败');
    },
...
function number_format(number, decimals, dec_point, thousands_sep) {
    number = (number + '').replace(/[^0-9+${symbol_escape}-Ee.]/g, '');
    ...
    if (s[0].length > 3) {
        s[0] = s[0].replace(/${symbol_escape}B(?=(?:${symbol_escape}d{3})+(?!${symbol_escape}d))/g, sep);
    }
    if ((s[1] || '').length < prec) {
        s[1] = s[1] || '';
...
function initInstitutionalFrameworkTreeMini(treeContainerId, getDataUrl, zTreeOnClick, options) {
    ...
    if (data) {
        ...
        var tree = ${symbol_dollar}('<ul></ul>').attr('id', treeId).addClass('ztree');
        ${symbol_dollar}(treeContainerId).html(tree);
        var treeData = evalJSON(data);
        ${symbol_dollar}.each(treeData, function (i, v) {
            v.isParent = true;
        });

        treeObj = ${symbol_dollar}.fn.zTree.init(${symbol_dollar}(tree), settings, treeData);
    }
    return treeObj;
}
;
...
function uploadfile(inputId, uploadAction, options) {
    if (uploadAction) {
        uploadAction += ';jsessionid=${symbol_dollar}{sessionId}';
    }
    var settings = ${symbol_dollar}.extend({
        'formData': {
            'id': ''
    ...
    if (!settings.queueID) {
        ...
        var queue = ${symbol_dollar}('<div></div>').attr('id', queueId);
        input.before(queue);
    }
...
function create_grid_processer(value, options) {
    ...
    var color = value > 70 ? '${symbol_pound}cc0000' : '${symbol_pound}7fb80e';
    return '<div style="width:100%;border:1px solid ${symbol_pound}ccc;">' +
            '<div style="width:' + value + '%;background:' + color + ';color:${symbol_pound}000">' + value + '%' + settings.tip + '</div>'
            + '</div>'
...
function show_outer_person_info_dialog(id) {
    ...
    var div = ${symbol_dollar}('<div></div>').css('padding', '30px 0 0 30px');
...
${symbol_dollar}.extend(${symbol_dollar}.fn.validatebox.defaults.rules, {
    dateLtOther: {
        validator: function (value, param) {
            var date1 = new Date(value.replace(/${symbol_escape}-/g, "${symbol_escape}/"));
            var date2 = new Date(${symbol_dollar}(param[0]).datebox('getValue').replace(/${symbol_escape}-/g, "${symbol_escape}/"));
 ...
 * @returns {{}}
 */
${symbol_dollar}.fn.serializeObject = function () {
    var o = {};
    var a = this.serializeArray();
    ${symbol_dollar}.each(a, function () {
...
function layout_center_close_current_tab(){
    var layout_center_tabs = ${symbol_dollar}('${symbol_pound}layout_center_tabs').size() > 0 ? ${symbol_dollar}('${symbol_pound}layout_center_tabs') : parent.${symbol_dollar}('${symbol_pound}layout_center_tabs');
